...
const parseArgs = (argv: readonly string[]): ParsedArgs => {
  ...
  for (let i = 0; i < argv.length; i += 1) {
    ...
    switch (arg) {
      ...
      case "--no-leader-drives": leaderDrives = false; break;
...
const main = async (): Promise<void> => {
  ...
  if (!args.request) {
    process.stderr.write(
      'Usage: helm "<request>" [--cli|--sdk|--mock] [--autonomous] [--no-team-mode]\n' +
        "             [--no-optimise] [--no-leader-drives] [--bare] [--model <id>] [--build --workspace <dir>] [--transcript]\n",
    );
    process.exitCode = 1;
    return;
  }
  ...
  try {
    const result = await runHelm({
      ...
      // Leader-driven is the default; only real runners can host the in-process tools.
      ...(args.leaderDrives && args.runner !== "mock" ? { leaderDrives: true } : {}),
